...
PLUGIN_NAMES = ("specflow",)
PLUGIN_PACKAGES = {"specflow": REPOSITORY_ROOT / "plugins" / "specflow"}
PLUGIN_MARKETPLACE_ROOT = REPOSITORY_ROOT
PLUGIN_MARKETPLACE_NAME = "specflow-local"
    ...
    try:
        ...
        if args.asset == "plugin":
            ...
            targets, selection = select_targets(
                plugin_config, requested, explicit, args.strict
            )
            ...
            if skill_config is None:
                raise SyncError(
                    f"project release configuration does not define {args.asset}"
                )
            ...
            targets, selection = select_targets(
                skill_config, requested, explicit, args.strict
            )
            outcomes.extend(selection)
            exit_code = manage_skill(
                skill_config, args.command, targets, args.dry_run, outcomes
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "asset", nargs="?", choices=("plugin", "skill"), help="要管理的资产类型"
    )
    parser.add_argument(
        "command", nargs="?", choices=("check", "list", "apply", "remove")
    )
    ...
    parser.add_argument(
        "--strict", action="store_true", help="写入前要求三个客户端均可用"
    )
    parser.add_argument(
    ...
    for config in configs:
        ...
        if command == "remove":
            plans: Sequence[ClientPlan] = [
                plan_plugin_removal(config, target, runner) for target in targets
            ]
        else:
